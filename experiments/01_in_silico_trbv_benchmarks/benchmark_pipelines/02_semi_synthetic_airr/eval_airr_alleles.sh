...
#   3. Run code/experiments/00_benchmark_utils/evaluate_allele_calls.py
#
# Directory Mapping:
...
TRBV_INDEX="${PACKAGE_ROOT}/data/ref/TRB_index.csv"
PY_SCRIPT="${PACKAGE_ROOT}/code/experiments/00_benchmark_utils/evaluate_allele_calls.py"

# ---------------- Arg Parsing ----------------
usage() {
  cat <<EOF
Usage:
  bash $0 --expr expr_AIRR1
  ...
  for pop in ${POPS//,/ }; do
    ...
    for gene in ${GENES//,/ }; do
      ...
      for infer_file in "${infer_files[@]}"; do
          ...
          # Match Seed ID from filename
          if [[ $filename =~ (ERR[0-9]+) ]]; then
              seed_id="${BASH_REMATCH[1]}"
              
              # Find corresponding Label File
              label_match=( "${LABEL_DIR}/genotype_"*"${seed_id}.csv" )
              ...
              if [[ ${#label_match[@]} -gt 0 ]]; then
                  ...
                  if [[ "$INSEC" == "T" ]]; then
                      out_prefix="${OUT_DIR}/eval_${method}_${seed_id}_${GENE}_intersect"
                  else
                      out_prefix="${OUT_DIR}/eval_${method}_${seed_id}_${GENE}"
                  fi
                  ...
                  # Execute
                  if "${CMD[@]}"; then
                      echo "    [OK] sample${seed_id}"
                      count=$((count + 1))
                  else
                      echo "    [FAIL] Python script failed for sample${seed_id}"
                  fi
              else
                  echo "    [WARN] No matching genotype file for sample${seed_id} in ${LABEL_DIR}"
              fi
          fi
      done
