...
#   2. Scans input: sample/{EXPR_ID}/{POP}/ERR*
...
# Python Script
PY_SCRIPT="${PACKAGE_ROOT}/code/experiments/00_benchmark_utils/clones2alleles.py"
...
for pop in ${POPS//,/ }; do
  ...
  for gene in ${GENES//,/ }; do
    ...
    for sample_path in "${sample_dirs[@]}"; do
        ...
        sample_base=$(basename "$sample_path")       # e.g. ERR123456

        # Find input clones file in the sample directory
        input_file=$(find_clones_file "$sample_path")

        if [[ -n "$input_file" ]]; then
            # Output Filename: infer_{sample_name}.{GENE}.csv
            ...
            if python "$PY_SCRIPT" \
                ...
                if [[ -f "$output_path" ]]; then
                     ...
                     echo "     [FAIL] ${sample_base}: Output file not created."
                fi
            else
                echo "     [FAIL] Python script error on ${sample_base}"
            fi
        else
            echo "     [SKIP] ${sample_base}: No clones file found."
        fi
    done
    shopt -u nullglob
  done # End Gene Loop
