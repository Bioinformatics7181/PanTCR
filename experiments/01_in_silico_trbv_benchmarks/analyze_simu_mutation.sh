...
REF_V="${SIM_REF_ROOT}/IMGT_TRBV_pro.tsv"
REF_J="${SIM_REF_ROOT}/IMGT_TRBJ_pro.tsv"
...
usage() {
  cat <<EOF
Usage:
  bash $0 --pop "AFR EUR" --expr expr_ScenarioA
...
for pop in ${POPS//,/ }; do
  ...
  # Input path: sample root / experiment ID / population.
  POP_INPUT_DIR="${SAMPLE_ROOT}/${EXPR_ID}/${pop}"
  
  # Output path: results root / mutations / experiment ID / population.
  ...
  for seed_path in "${POP_INPUT_DIR}"/seed*; do
    ...
    # Current layout: the seed folder directly contains the generated files.
    input_dir="$seed_path"
