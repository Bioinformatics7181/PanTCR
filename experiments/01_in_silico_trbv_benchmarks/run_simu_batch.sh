...
# Defaults
# =========================
# Support multiple populations by default.
...
MUT_PROB=0.0           # V-gene point-mutation probability for Scenario C
J_MUT_PROB=0.0         # J-gene point-mutation probability
MUT_RANGE_V=70         # mutate within the first N bases for V genes
MUT_RANGE_J=28         # mutate within the last N bases for J genes
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --pop) POPS="$2"; shift 2 ;; # Input can be a list such as "AFR EUR".
    ...
    --mut_prob|--mut-prob) MUT_PROB="$2"; shift 2 ;;
    --j_mut_prob|--j-mut-prob) J_MUT_PROB="$2"; shift 2 ;;
    --mut_range_V|--mut-range-v) MUT_RANGE_V="$2"; shift 2 ;;
    --mut_range_J|--mut-range-j) MUT_RANGE_J="$2"; shift 2 ;;
    ...
    -h|--help)
      echo "Usage:"
      echo "  $0 --pop \"AFR EUR\" --seed_start 0 --n 5 --expr expr_ScenarioA"
      exit 0
      ;;
    *)
...
SIM_REF_ROOT="${PANTCR_SIM_REF_ROOT:-${PACKAGE_ROOT}/data/ref}"
...
# =========================
# Use one parent container for each experiment ID.
EXPR_DIR="${OUT_ROOT}/${EXPR_ID}"
mkdir -p "$EXPR_DIR"

# Save the experiment config under EXPR_DIR, parallel to population folders.
EXPERIMENTS_TSV="${EXPR_DIR}/experiments_config.tsv"

if [[ ! -f "$EXPERIMENTS_TSV" ]]; then
  printf "timestamp\texpr_id\tpop\tseed\texclude_default\tfunctionality\tmut_prob\tj_mut_prob\tmut_range_V\tmut_range_J\tnc\tnr\talpha\tp_degraded\tp_degraded_sd\tlen_mean\tlen_sd\tlen_min\tlen_max\tcut_intact_mean\tcut_intact_sd\tcut_max_intact\tcut_degraded_mean\tcut_degraded_sd\tcut_max\tc_max\tmin_keep_len\tart_ss\tart_l\tart_c\tart_m\tart_s\tmixcr_preset\tmixcr_threads\n" > "$EXPERIMENTS_TSV"
fi
...
log_one_seed () {
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    ...
    "$MUT_PROB" "$J_MUT_PROB" "$MUT_RANGE_V" "$MUT_RANGE_J" \
    "$NC" "$NR" "$ALPHA" \
...
run_one_seed () {
  ...
  # Directory structure: sample root / experiment ID / population / seed.
  ...
  # 1) select genotype
  python "${SIMULATION_CODE_ROOT}/select_genotype.py" \
    -i "$(resolve_ref_path "${INPUT_REF}")" \
    -p "$pop" \
    -o "." \
    -s "$seed" \
    --prefix "genotype" \
    ...
    -d "$EXCLUDE_DEFAULT" \
    --mut_prob "$MUT_PROB" \
    --j_mut_prob "$J_MUT_PROB" \
    --mut_range_V "$MUT_RANGE_V" \
    --mut_range_J "$MUT_RANGE_J"
  ...
  python "${SIMULATION_CODE_ROOT}/simulate_repertoire.py" \
    --genotype "$geno_csv" \
    --dict "$(resolve_ref_path "${CDR3_DICT}")" \
  ...
  python "${SIMULATION_CODE_ROOT}/simulate_bias_advanced.py" \
    -i "$rep_csv" \
...
# =========================
# Split the population list by spaces or commas and iterate.
