...
#   4. Process: code/pantcr/infer_genotype_bayes.py
#   5. Output: 
...
PY_SCRIPT="${PACKAGE_ROOT}/code/pantcr/infer_genotype_bayes.py"

# ---------------- Arg Parsing ----------------
usage() {
  cat <<EOF
Usage:
  bash $0 --expr expr_ScenarioA --k 5 --min-naive 1 [--no-prior]
...
Options:
  ...
  --penalty-k     Extra K=2 penalty for synthetic runs (default: ${PENALTY_K})
  --pi-min        Minimum mixture proportion for synthetic runs (default: ${PI_MIN})
