...
echo "[INIT] Package root: $PACKAGE_ROOT"
...
show_help() {
    ...
    echo "  --mut-prob FLOAT        V-gene point mutation probability for Scenario C (default: ${SIM_MUT_PROB})"
    echo "  --j-mut-prob FLOAT      J-gene point mutation probability (default: ${SIM_J_MUT_PROB})"
    echo "  --mut-range-v INT       V mutation window from sequence start (default: ${SIM_MUT_RANGE_V})"
    echo "  --mut-range-j INT       J mutation window from sequence end (default: ${SIM_MUT_RANGE_J})"
    echo ""
    ...
    echo "  --art-ss STR            ART sequencing system (default: ${SIM_ART_SS})"
    echo "  --art-l INT             ART read length (default: ${SIM_ART_L})"
    echo "  --art-c INT             ART read count multiplier (default: ${SIM_ART_C})"
    echo "  --art-m INT             ART 'm' param (default: ${SIM_ART_M})"
    echo "  --art-s INT             ART 's' param (default: ${SIM_ART_S})"
    echo "  --mixcr-preset STR      MiXCR analyze preset (default: ${SIM_MIXCR_PRESET})"
    echo "  --mixcr-threads INT     MiXCR threads (default: ${SIM_MIXCR_THREADS})"
    echo ""
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --mut-prob|--mut_prob) SIM_MUT_PROB="$2"; shift ;;
        --j-mut-prob|--j_mut_prob) SIM_J_MUT_PROB="$2"; shift ;;
        --mut-range-v|--mut_range_V) SIM_MUT_RANGE_V="$2"; shift ;;
        --mut-range-j|--mut_range_J) SIM_MUT_RANGE_J="$2"; shift ;;
        ...
        --art-ss)           SIM_ART_SS="$2"; shift ;;
        --art-l)            SIM_ART_L="$2"; shift ;;
        --art-c)            SIM_ART_C="$2"; shift ;;
        --art-m)            SIM_ART_M="$2"; shift ;;
        --art-s)            SIM_ART_S="$2"; shift ;;
        --mixcr-preset)     SIM_MIXCR_PRESET="$2"; shift ;;
        --mixcr-threads)    SIM_MIXCR_THREADS="$2"; shift ;;
...
echo "   Mutation       : V=${SIM_MUT_PROB} J=${SIM_J_MUT_PROB} V_range=${SIM_MUT_RANGE_V} J_range=${SIM_MUT_RANGE_J}"
...
if should_run_step 1; then
    ...
    bash run_simu_batch.sh \
      ...
      --mut_prob "${SIM_MUT_PROB}" \
      --j_mut_prob "${SIM_J_MUT_PROB}" \
      --mut_range_V "${SIM_MUT_RANGE_V}" \
      --mut_range_J "${SIM_MUT_RANGE_J}" \
      --nc "${SIM_NC}" \
      --nr "${SIM_NR}" \
      --alpha "${SIM_ALPHA}" \
      ...
      --art_m "${SIM_ART_M}" \
      --art_s "${SIM_ART_S}" \
      --mixcr_preset "${SIM_MIXCR_PRESET}" \
      --mixcr_threads "${SIM_MIXCR_THREADS}" \
