...
# -*- coding: utf-8 -*-
"""
PanTCR graph-guided Bayesian genotype inference for mature V/J allele calling.

Logic:
  - Uses sample-level retained observations and optional population-aware graph priors.
Output:
  - Reports one or two inferred allele sequences per assigned V/J gene.
  - Appends mixture proportion and diagnostic fields as trailing columns.
...
# ----------------------------
# Missing-region completion (conditional MAP dynamic programming under the Markov prior)
# ----------------------------
...
def _allowed_states_at_pos(ref: str,
    ...
    """
    Build candidate state list at a position for graph-guided completion.
...
def graph_guided_complete_amap(ref: str,
                               base_amap: Dict[int, str],
                               covered: Set[int],
    ...
    for pos in range(0, L - 1):
        ...
        for v in allowed[pos + 1]:
            ...
            dp2[v] = best
            # best_u should exist because dp non-empty; still guard
    ...
    # Convert to amap: only store positions where state differs from ref (like existing amap convention)
    filled: Dict[int, str] = {}
...
def amap_to_sequence(ref: str, amap: Dict[int, str]) -> str:
    """Materialize a full-length sequence from amap/ref, then remove '-' deletions (keeps insertion strings)."""
    out = []
...
def build_evidence_items(sample_df: pd.DataFrame, ref: str, min_naive: int = 2) -> Tuple[List[EvidenceItem], Set[str], Set[int]]:
    ...
    """
    df = sample_df.copy()
    # Apply the naive-diversity support threshold used by the current workflow.
    ...
    for r in df.itertuples(index=False):
        ...
        for i in range(mlen):
            ...
            # Effective evidence strength for this split (simple, stable)
            w = max(0.0, d + 2.0 * n)
            if w <= 0:
                continue
...
def log_p_e_given_allele(e: EvidenceItem, ref: str, amap: Dict[int, str], eps: float) -> float:
    ...
    # mismatch assumes 4-way; keep simple
...
def build_candidate_token_lists(ref: str, mut_infos: Set[str], max_candidates: int = 40) -> List[List[MutationToken]]:
    ...
    Builds candidate token sets for the graph-constrained inference step.
    """
...
def estimate_pi_grid(logp1: np.ndarray, logp2: np.ndarray, weights: np.ndarray, pi_min: float) -> Tuple[float, float]:
    ...
    for pi in pis:
        if pi_min > 0 and (pi < pi_min or pi > 1.0 - pi_min):
            # still evaluate for likelihood, but we mostly care about feasible region for hetero
            pass
...
def infer_family(fam: str, sdf: pd.DataFrame, node_df: pd.DataFrame, edge_df: Optional[pd.DataFrame],
    ...
    # Build allele maps and priors after graph-guided completion to avoid reference bias in missing regions.
    cand = []
    for toks in cand_token_lists:
        amap = allele_map_from_tokens(ref, toks)
        mi = tokens_to_mutinfo(toks)
        amap_filled = graph_guided_complete_amap(ref, amap, covered, nodep, edgep)
...
def main():
    ...
    ap.add_argument("--penalty_K", type=float, default=3.0,
                    help="Extra penalty for K=2 vs K=1 (default: 3.0).")
    ap.add_argument("--out", required=True)

    # optional but safe: if not provided, behaves like default robust setting
