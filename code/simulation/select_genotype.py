...
def str_to_bool(val):
    """
    Convert string value (TRUE/FALSE) to Python boolean.
    """
...
def apply_mutation(sequence, prob, start_idx, end_idx):
    """
    Apply random point mutation based on probability and specific index range.
    
    Args:
        sequence (str): The DNA sequence.
        prob (float): Mutation probability (0.0 to 1.0).
        start_idx (int): Start index of the mutation window (inclusive).
        end_idx (int): End index of the mutation window (exclusive).

    Returns:
        tuple: (new_sequence, is_mutated, mutation_info_str)
        mutation_info_str format: S<OldBase><Pos><NewBase> (e.g., SA0T)
    """
    # 1. Check probability
    ...
    # 3. Adjust limits to sequence bounds
    # Ensure start is at least 0
    actual_start = max(0, start_idx)
    # Ensure end is at most seq_len
    actual_end = min(seq_len, end_idx)

    # If the window is invalid (e.g. start >= end), return original
    if actual_start >= actual_end:
        return sequence, False, ""

    # 4. Randomly select position within the [actual_start, actual_end)
    pos = np.random.randint(actual_start, actual_end)

    # 5. Get original base
    original_base = sequence[pos].upper()
    bases = ['A', 'T', 'C', 'G']
    
    # 6. Select new base (ensure it is different from original)
    candidates = [b for b in bases if b != original_base]
    if not candidates: 
        # Fallback if original base is not ATCG (e.g., 'N')
        candidates = bases
        
    new_base = np.random.choice(candidates)
    ...
    # 8. Generate info string: Prefix 'S' + Old + Pos + New (e.g., SA0T)
    mutation_info = f"S{original_base}{pos}{new_base}"
    
    return new_sequence, True, mutation_info

def select_alleles(input_file, population, seed, output_dir, prefix, functionality, exdefault_val, mut_prob, mut_prob_j, mut_range_v, mut_range_j):
    ...
    functionality_filter = str(functionality or "").strip()
    if functionality_filter:
        ...
        if func_col:
            print(f"Filtering alleles with {func_col} = '{functionality_filter}'...")
            initial_count = len(df)
            # Force string comparison to avoid type mismatch
            df = df[df[func_col].astype(str).str.strip() == functionality_filter]
            print(f"  Kept {len(df)} out of {initial_count} alleles.")
            
            if len(df) == 0:
                print("Error: No alleles left after functionality filtering!")
                sys.exit(1)
        else:
            print(f"Error: --functionality specified as '{functionality_filter}', but column 'functionality' not found.")
            sys.exit(1)
    ...
    if mut_prob > 0 or mut_prob_j > 0:
        print(f"Mutation enabled: Prob_V={mut_prob}, Prob_J={mut_prob_j}")
        print(f"  - V Genes: First {mut_range_v} bases (0 to {mut_range_v})")
        print(f"  - J Genes: Last {mut_range_j} bases (Right aligned)")
    else:
        print("Mutation disabled (Prob=0).")
    ...
    for gene, group in grouped:
        ...
        # Handle case where filtering left no alleles for a specific gene
        if len(indices) == 0:
            continue
        ...
        raw_seq_A = sequences[idx_A]
        raw_seq_B = sequences[idx_B]
        allele_name_A = alleles[idx_A]
        allele_name_B = alleles[idx_B]
        
        # Determine mutation range based on Gene Name
        # Logic: 
        #   V -> Start from 0, range = mut_range_v
        #   J -> Start from (Len - mut_range_j), range = to end
        
        gene_upper = gene.upper()
        
        # Default: no mutation range if neither V nor J (safety fallback)
        temp_prob = 0
        start_A, end_A = 0, 0
        start_B, end_B = 0, 0
        
        if 'V' in gene_upper:
            # V logic: Left aligned (0 to 70)
            temp_prob = mut_prob

            len_A = len(raw_seq_A)
            len_B = len(raw_seq_B)
            
            start_A, end_A = 0, mut_range_v
            start_B, end_B = 0, mut_range_v
            
        elif 'J' in gene_upper:
            # J logic: Right aligned (Last 28 bases)
            temp_prob = mut_prob_j

            len_A = len(raw_seq_A)
            len_B = len(raw_seq_B)
            
            start_A, end_A = len_A - mut_range_j, len_A
            start_B, end_B = len_B - mut_range_j, len_B
        
        # Mutate Sequence A
        seq_A, mutated_A, mut_info_A = apply_mutation(raw_seq_A, temp_prob, start_A, end_A)
        if mutated_A:
            allele_name_A += "_mut"
            
        # Mutate Sequence B
        seq_B, mutated_B, mut_info_B = apply_mutation(raw_seq_B, temp_prob, start_B, end_B)
        if mutated_B:
            allele_name_B += "_mut"
        ...
        row = {
            'gene': gene,
            'allele_A': allele_name_A,
            'seq_A': seq_A,
            'allele_B': allele_name_B,
            'seq_B': seq_B,
            'mut_A': mut_info_A,   # New Column
            'mut_B': mut_info_B    # New Column
        }
...
def main():
    ...
    parser.add_argument(
        ...
        default=0.0,
        type=float,
        help="Probability of applying a point mutation for V (0.0 to 1.0). Default is 0."
    )
    
    parser.add_argument("--j_mut_prob", 
        default=0.0, 
        type=float, 
        help="Probability of applying a point mutation for J (0.0 to 1.0). Default is 0.")

    parser.add_argument(
        "--mut_range_V",
        default=70,
        type=int,
        help="Range for V mutation (first N nucleotides). Default is 70 (from start 0 to 70)."
    )
    ...
    parser.add_argument(
        ...
        default=28,
        type=int,
        help="Range for J mutation (last N nucleotides). Default is 28 (from end-28 to end)."
    )
    
    args = parser.parse_args()
    
    select_alleles(args.input, args.pop, args.seed, args.output_dir, args.prefix, 
                   args.functionality, args.exdefault, 
                   args.mut_prob, args.j_mut_prob, args.mut_range_V, args.mut_range_J)
