...
def infer_family(fam: str, sdf: pd.DataFrame, node_df: pd.DataFrame, edge_df: Optional[pd.DataFrame],
    ...
    # build allele maps and priors (prior is evaluated on Viterbi-completed amap to avoid reference bias in missing regions)
    cand = []
...
def main():
    ...
    ap.add_argument("--penalty_K", type=float, default=1.0,
                    help="Extra penalty for K=2 vs K=1 (kept for CLI compatibility; default 1.0).")
