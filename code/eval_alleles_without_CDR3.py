...
def main():
    ...
    for _, row in genotype_filtered.iterrows():
        ...
        if a_seq not in X_dict:
            X_dict[str(a_seq).strip().upper()] = set()
        X_dict[a_seq].add(a_name)

        if b_seq not in X_dict:
            X_dict[str(b_seq).strip().upper()] = set()
        X_dict[b_seq].add(b_name)
